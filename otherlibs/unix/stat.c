...
CAMLprim value NOINLINE unix_stat(value path)
{
  CAMLparam1(path);
  int ret;
  struct stat buf;
...
CAMLprim value NOINLINE unix_lstat(value path)
{
  CAMLparam1(path);
  int ret;
  struct stat buf;
...
CAMLprim value NOINLINE unix_fstat(value fd)
{
  int ret;
  struct stat buf;
  caml_enter_blocking_section();
...
CAMLprim value NOINLINE unix_stat_64(value path)
{
  CAMLparam1(path);
  int ret;
  struct stat buf;
...
CAMLprim value NOINLINE unix_lstat_64(value path)
{
  CAMLparam1(path);
  int ret;
  struct stat buf;
...
CAMLprim value NOINLINE unix_fstat_64(value fd)
{
  int ret;
  struct stat buf;
  caml_enter_blocking_section();
