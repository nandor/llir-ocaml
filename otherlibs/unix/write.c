...
CAMLprim value unix_write(value fd, value buf, value vofs, value vlen) NOINLINE
{
...
CAMLprim value unix_single_write(value fd, value buf, value vofs, value vlen) NOINLINE
{
  long ofs, len;
  int numbytes, ret;
  char iobuf[UNIX_BUFFER_SIZE];
